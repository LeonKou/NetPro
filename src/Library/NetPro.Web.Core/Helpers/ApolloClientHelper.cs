...
namespace NetPro.Web.Core.Helpers
{
	public static class ApolloClientHelper
	{
		...
		public static async Task GetNameSpaceType<IConfig>(this IConfiguration configuration, string namespaceName) where IConfig : class
		{
			var apollomanager = EngineContext.Current.Resolve<IConfigManager>();
			var config = await apollomanager.GetConfig(namespaceName);
